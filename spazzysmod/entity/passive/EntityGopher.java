...
public class EntityGopher extends EntityTameable {
	...
	protected void entityInit() {
		super.entityInit();
		this.dataWatcher.addObject(25, new Float(this.func_110143_aJ()));
	...
	public float getTailRotation() {
		return this.isAngry() ? 1.5393804F
				: (this.isTamed() ? (0.55F - (20 - this.dataWatcher
						.getWatchableObjectInt(25)) * 0.02F) * (float) Math.PI
						: ((float) Math.PI / 5F));
	}
			...
			if (itemstack != null) {
				if (Item.itemsList[itemstack.itemID] instanceof ItemFood) {
					...
					if (itemfood.isWolfsFavoriteMeat()
							&& this.dataWatcher.getWatchableObjectInt(25) < 20) {
	...
	public boolean isBreedingItem(ItemStack par1ItemStack) {
		return par1ItemStack == null ? false : (!(par1ItemStack.itemID == Item.carrot.itemID) ? false: true);
	}
	...
	protected void updateAITick() {
		this.dataWatcher.updateObject(25, Float.valueOf(this.func_110143_aJ()));
	}
