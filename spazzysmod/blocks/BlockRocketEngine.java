...
public class BlockRocketEngine extends Block {
	...
	public boolean onBlockActivated(World par1World, int x, int y, int z,
			...
			float par9) {
		if (par1World.getBlockId(x, y + 1, z) == Block.blockIron.blockID
				&& par1World.getBlockId(x, y + 2, z) == Block.glass.blockID
				&& par1World.getBlockId(x, y + 3, z) == Block.blockIron.blockID
			...
			if (par5EntityPlayer.dimension == -1) {
				par5EntityPlayer
						.addChatMessage("Can't lift off from the Nether!");
			} else {
				if (par5EntityPlayer.getClass() != EntityClientPlayerMP.class) {
					Minecraft.getMinecraft().displayGuiScreen(
							new GuiPlanets(par5EntityPlayer));
	...
	public Icon getIcon(int par1, int par2) {
		return par1 == 1 ? this.top
				: (par1 == 0 ? this.bottom : this.blockIcon);
	}
	...
	public void registerIcons(IconRegister par1IconRegister) {
		...
		this.bottom = par1IconRegister.registerIcon("spazzysmod:" + "bottom"
				+ this.getUnlocalizedName().substring(5));

		this.top = par1IconRegister.registerIcon("spazzysmod:" + "top"
				+ this.getUnlocalizedName().substring(5));
	}
}
