...
public class EntityExplosiveArrow extends Entity {
	...
	public EntityExplosiveArrow(World par1World, double par2, double par4,
			double par6) {
		super(par1World);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	...
	public EntityExplosiveArrow(World par1World,
		...
		this.setLocationAndAngles(
				...
				par2EntityLivingBase.posY
						+ par2EntityLivingBase.getEyeHeight(),
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					f2 = MathHelper.sqrt_double(this.motionX * this.motionX
							...
							* this.motionZ);
					int i1 = MathHelper.ceiling_double_int(f2
							* this.damage);
							...
							if (!this.worldObj.isRemote) {
								...
								for (int z = 0; z < 1; z++) {
									this.worldObj.createExplosion(this,
											this.posX, this.posY, this.posZ, 3,
											true);
									this.setDead();
								}
							}
			...
			if (this.getIsCritical()) {
				for (l = 0; l < 4; ++l) {
					this.worldObj.spawnParticle("crit", this.posX
							+ this.motionX * l / 4.0D, this.posY
							+ this.motionY * l / 4.0D, this.posZ
							+ this.motionZ * l / 4.0D, -this.motionX,
			...
			if (this.isInWater()) {
				for (int j1 = 0; j1 < 4; ++j1) {
					...
					this.worldObj.spawnParticle("bubble", this.posX
							- this.motionX * f3, this.posY
							- this.motionY * f3, this.posZ
							- this.motionZ * f3, this.motionX,
